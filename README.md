...
- [x402 Entity-ID Resolver](https://x402-entity-resolver-production.up.railway.app) - Canonical cross-registry entity-ID resolver for AI agents. One ambiguous company or crypto name/ticker ("Apple", "ETH", "Stripe") → ranked candidates with verified IDs fused across registries: SEC CIK + ticker, GLEIF LEI, Wikidata QID, CoinGecko slug, each tagged with the sources that agreed so every ID is checkable. The exact cross-registry lookup LLMs can't do reliably. $0.005 USDC per call on Base mainnet, gasless (EIP-3009), no API key. v2 with Bazaar discovery extension. ([OpenAPI](https://x402-entity-resolver-production.up.railway.app/openapi.json)) ([llms.txt](https://x402-entity-resolver-production.up.railway.app/llms.txt)) ([.well-known/x402](https://x402-entity-resolver-production.up.railway.app/.well-known/x402))
